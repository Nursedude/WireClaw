...
- **AI Agent** - agentic loop with 24 tools, up to 5 iterations per message
- **Local LLM** - use a local server (Ollama, llama.cpp) over HTTP instead of cloud API
- **OpenClaw Integration** - [OpenClaw](https://github.com/openclaw) (or any NATS client) can execute tools directly on the ESP32 without involving WireClaw's LLM. Flat JSON protocol, device discovery, 23 tools available. Includes a skill and wrapper script.
