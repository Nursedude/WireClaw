...
All 20 tools are available via `tool_exec` (except `remote_chat` - see [Security](#security)):
...
- **`SKILL.md`** - full reference: protocol, all 20 tools with parameters, constraints (one action per rule, edge-triggered behavior, clock sensors), cross-domain patterns, and worked examples. Derived from WireClaw's actual system prompt so OpenClaw generates correct tool calls on the first try.
